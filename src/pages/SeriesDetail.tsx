...
export default function SeriesDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getSeriesInfo(credentials, Number(id))
      .then((data) => {
        setSeriesInfo(data);
  ...
  const handlePlayEpisode = (episode: Episode) => {
    ...
    addToHistory({
      ...
      episodeInfo: `S${episode.season}E${episode.episode_num}`,
    });
  ...
  if (!seriesInfo) {
    return (
      <div className="text-center py-12 text-muted-foreground">
        ...
        <br />
        <button onClick={() => navigate('/series')} className="text-primary mt-4 underline">Voltar</button>
      </div>
    );
  }
    ...
    <div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {seasons.map(s => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                  selectedSeason === s ? 'gradient-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'
                }`}
              >
                Temporada {s}
              </button>
          ...
          {currentEpisodes.map((ep) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {ep.info?.duration && (
                  <p className="text-xs text-muted-foreground mt-0.5">{ep.info.duration}</p>
                )}
              </div>
        ...
        {currentEpisodes.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">Nenhum episódio encontrado nesta temporada.</div>
        )}
      </motion.div>
    </div>
  );
