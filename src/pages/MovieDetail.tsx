...
import { getVodInfo, getVodStreams, VodStream, buildStreamUrl } from '@/services/xtreamApi';
...
export default function MovieDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      getVodStreams(credentials).then(movies => movies.find(m => m.stream_id === Number(id)) || null),
      getVodInfo(credentials, Number(id)).catch(() => null),
    ]).then(([m, i]) => {
      setMovie(m);
      setInfo(i);
  ...
  if (!movie) {
    return (
      <div className="text-center py-12 text-muted-foreground">
        ...
        <br />
        <button onClick={() => navigate('/movies')} className="text-primary mt-4 underline">Voltar</button>
      </div>
    );
  }
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="aspect-[2/3] rounded-xl overflow-hidden bg-secondary">
            {movie.stream_icon ? (
              <img src={movie.stream_icon} alt={movie.name} className="w-full h-full object-cover" />
            ) : (
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="flex flex-wrap gap-3 text-sm text-muted-foreground">
            {movie.rating && (
              <span className="flex items-center gap-1"><Star className="w-4 h-4 text-primary" /> {movie.rating}</span>
            )}
          ...
          {genre && (
            <div className="flex flex-wrap gap-2">
              {genre.split(',').map((g: string) => (
                <span key={g.trim()} className="px-2 py-1 rounded-full bg-secondary text-xs text-foreground">
                  {g.trim()}
                </span>
              ))}
            </div>
